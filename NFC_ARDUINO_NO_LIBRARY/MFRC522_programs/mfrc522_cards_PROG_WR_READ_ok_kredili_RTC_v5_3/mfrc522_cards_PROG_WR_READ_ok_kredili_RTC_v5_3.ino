...
byte uid[7];
byte uidSize=4;
const byte Uuidsize=4;
byte esonUID[Uuidsize] = {0};
...
/* eski anticoll çalışan 4 byte için 
// Anticoll (4 byte + BCC)
bool mifareAnticoll(byte *uid4) {
  ...
  byte bcc = back[0]^back[1]^back[2]^back[3];// checksum 
...
void kartOku() {
  ...
  //mifareAnticoll(byte *uid, byte &uidSize) {
...
bool ayniKartMi(byte *uid, byte uidSize2)
{
  if (!sonUIDVar) return false;

  for (byte i=0; i<uidSize2; i++)
  {
...
void K_kart_yaz(byte *uid, int kredi, String zamanDamgasi, byte durum,int dusulen)
{
  ...
  //uint8_t uid[7] = {0x04,0xAA,0xBB,0xCC,0xDD,0x11,0x22};
  //memset(yeniLog.uid, 0, 7);        // önce alanı temizle
  //memcpy(yeniLog.uid, uid, 7); // gelen uid pointer'dan kopyala
